...
struct ContentView: View {
    ...
    private var header: some View { HStack { VStack(alignment: .leading, spacing: 5) { Text("TOYOTA LAND CRUISER PARTS").font(.caption2.bold()).tracking(1.4).foregroundStyle(orange); Text("Build it.\nExplore more.").font(.system(size: 42, weight: .heavy, design: .rounded)).foregroundStyle(ink); Text("Live catalog results, saved builds, and fitment-friendly browsing.").font(.subheadline).foregroundStyle(.secondary) }; Spacer(); Image(systemName: "mountain.2.fill").font(.system(size: 42)).foregroundStyle(olive).padding(16).background(ink, in: RoundedRectangle(cornerRadius: 18)) } }
    private var searchBar: some View { HStack { Image(systemName: "magnifyingglass").foregroundStyle(.secondary); TextField("Search live parts, SKU, or keyword…", text: $search).submitLabel(.search).onSubmit { selectedSeries = ""; store.load(search) }; Button("Search") { selectedSeries = ""; store.load(search) }.buttonStyle(.borderedProminent).tint(orange) }.padding(8).background(.white, in: RoundedRectangle(cornerRadius: 14)).shadow(color: ink.opacity(0.08), radius: 12, y: 5) }
    private var chipBar: some View { ScrollView(.horizontal, showsIndicators: false) { HStack { ForEach(chips, id: \.self) { chip in Button(chip) { selectedSeries = chip; search = chip == "Fresh parts" ? "" : chip; store.load(search) }.font(.caption.bold()).padding(.horizontal, 14).padding(.vertical, 10).foregroundStyle(selectedSeries == chip ? .white : ink).background(selectedSeries == chip ? olive : .white, in: Capsule()).overlay(Capsule().stroke(olive.opacity(0.25))) } } } }
...
struct ProductCard: View {
    ...
    var body: some View { NavigationLink(value: product) { VStack(alignment: .leading, spacing: 8) { AsyncImage(url: CatalogClient.shared.imageURL(product.image)) { phase in if let image = phase.image { image.resizable().scaledToFit() } else { Image(systemName: "shippingbox.fill").font(.largeTitle).foregroundStyle(olive.opacity(0.5)) } }.frame(maxWidth: .infinity).frame(height: 115).background(Color.white, in: RoundedRectangle(cornerRadius: 12)); Text(recommended ? "RECOMMENDED" : "LIVE CATALOG").font(.caption2.bold()).foregroundStyle(olive); Text(product.name).font(.headline).foregroundStyle(ink).lineLimit(2).multilineTextAlignment(.leading); Text(product.price.isEmpty ? "See current price" : product.price).font(.subheadline.bold()).foregroundStyle(ink); Button(store.isSaved(product) ? "Saved" : "+ Save") { store.toggleSaved(product) }.font(.caption.bold()).buttonStyle(.bordered).tint(store.isSaved(product) ? olive : orange) } }.buttonStyle(.plain).padding(10).background(.white, in: RoundedRectangle(cornerRadius: 15)).overlay(RoundedRectangle(cornerRadius: 15).stroke(recommended ? olive.opacity(0.35) : Color.black.opacity(0.06))) }
}
